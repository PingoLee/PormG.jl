...
- **Flag a new comparator or escape in the migration planner's field diff** — a `typeof` / `isa` gate on a field struct, a reconciliation branch in `_alter_table_fields`, or a second answer to "is this the same column?" anywhere. #507 phase 1 replaced all of them with one canonical column IR, so this is now a **regression against the IR**, not a case to route onward: the fix belongs in `Migrations.column_spec` / `parse_canonical_type`, and a difference the compiler cannot express is a missing fact in `ColumnSpec`, not an exception beside it. Also flag a new `NON_DB_ATTRS` entry that hides a fact some renderer does emit. Background: [`../pormg-migrations-development/SKILL.md`](../pormg-migrations-development/SKILL.md) → *Planner internals: column identity*
- **Flag a plan ACTION that re-inspects the field structs instead of reading the typed delta.** The sibling of the rule above, and the one #507 phase 2 added: since then a `ColumnDelta` is the whole input to every decision the plan makes about a column, so `field.null` / `field.unique` / `hasproperty(field, :generated)` used to *decide whether to emit a statement*, a second `_fk_constraint_action`, or a private copy of the alteration path inside the rename branch are all regressions — not local bugs to patch where they sit. Reading a field to render SQL **text** (a column type, a `USING` cast) is fine and expected; reading one to re-decide is the flag. Four bugs in one week came from action sites holding their own opinion — #498, #504, #514, #515 — which is why the shape matters more than any one instance. The fix is to read the delta; if the delta cannot express the fact, the missing fact belongs in `ColumnSpec`. Contract: [`../pormg-migrations-development/SKILL.md`](../pormg-migrations-development/SKILL.md) → *From delta to actions*
