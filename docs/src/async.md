...
!!! warning "Interrupting a query *inside* an open transaction does not return promptly — and do not press `Ctrl+C` twice"
    A `Ctrl+C` while a query is in flight **within** a `run_in_transaction` body behaves differently. Every query inside the block runs on the transaction's single pinned connection, whose lifecycle the transaction owns rather than the query — so the background recovery above deliberately does not run for it. Instead the cleanup `ROLLBACK` is issued on that same connection and waits behind the very query you interrupted: on PostgreSQL it blocks on the driver's per-connection lock, on SQLite it queues on the single worker. **Control comes back only once the interrupted query finishes on its own** — measured on PostgreSQL as almost exactly the query's remaining runtime.

    For a *single* interrupt that wait is the whole cost: the `ROLLBACK` does run once it gets the connection, the transaction is undone, and the slot goes back to the pool clean on both backends.

    **A second `Ctrl+C` during that wait is the unsafe move**, and the long wait is exactly what tempts people into it. It lands on the blocked `ROLLBACK`, which is then treated as a failed rollback and triggers connection renewal *while the first query is still running* — on SQLite that closes a database handle the worker is still inside, and the `ROLLBACK` never runs at all. Wait it out, or avoid the situation: keep transactions short and run the slow query before the block rather than inside it.
