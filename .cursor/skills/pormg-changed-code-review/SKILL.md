...
In the final pass, review `docs`, `ext`, `.cursor`, `db`, and any remaining changed paths for:
...
1. Read `.cursor/rules/pormg-general.mdc` to get the current architecture checkpoints and subsystem map before starting — this ensures heuristics cover newly added subsystems automatically.
...
Always re-read `.cursor/rules/pormg-general.mdc` (step 1 of Review Method) so that new subsystems added to the architecture checkpoints are automatically included in the review. The bullets below are permanent baselines, not an exhaustive list.
...
- When a new subsystem file appears in `src/` that is not yet listed in `pormg-general.mdc`, flag it as an architecture-checkpoint gap
