...
#
# The bare-`IDField` key arm is skipped, and the reason is narrower than it looks — it is NOT that
...
function _expression_default_finding(table_name, col_name, raw_default, ctype, is_pk::Bool,
  ...
  _key_arm(is_pk, ctype, has_reference) === :id_pk && return nothing
...
function _sqlite_expression_default_findings(db::PormGSQLite;
  ...
  for trow in eachrow(tables)
    ...
    for crow in eachrow(cols)
      ...
      # The reader's own derivation of the type, verbatim: upper-cased and stripped.
      ctype = parse_canonical_type(uppercase(String(strip(String(something(crow.type, ""))))), db)
