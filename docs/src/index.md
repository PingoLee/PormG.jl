...
*   **DecimalField Precision**: Underpinned by `NUMERIC` types, `DecimalField(max_digits, decimal_places)` prevents float drift in round-trips (e.g. `99.99` is retrieved precisely as `99.99` in `Decimals.Decimal` format). `.list(:json)` carries that out rather than narrowing it: the column serializes as a JSON number holding the digits the value has, not re-rounded through a `Float64`. On SQLite the guarantee is bounded by the engine — `NUMERIC` affinity converts on write and keeps fifteen significant digits exactly — so PormG refuses a `DecimalField` wider than `max_digits = 15` there (`makemigrations` raises `BackendCapabilityError`) instead of creating a column that would silently round, and reads every column it does create back as a `Decimals.Decimal` too; see [Serializing rows to JSON](read/index.md#Serializing-rows-to-JSON).
