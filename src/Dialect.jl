...
using DataFrames
using LibPQ
import ..PormG: SQLConn, SQLType, SQLInstruction, SQLTypeQ, SQLTypeQor, SQLTypeF, SQLTypeOper, SQLObject, AbstractModel, PormGModel, PormGField, sqlite_date_format_map
...
function CONCAT(column::Array{String, 1}, format::Dict{String, Any}, conn::LibPQ.Connection)
    return "CONCAT($(join(columns, ",\n")))"
end
function CONCAT(column::Array{String, 1}, format::Dict{String, Any}, conn::SQLite.DB)
    return "($(join(columns, "||\n")))"
end
...
function CASE(column::Vector{String}, format::Dict{String, Any}, conn::Union{LibPQ.Connection,SQLite.DB})
...
function WHEN(column::Vector{String}, format::Dict{String, Any}, conn::Union{LibPQ.Connection,SQLite.DB})
    return "WHEN $(join(column, " ")) THEN $(format["value"])"
end
...
# SELECT 
#     dash_dim_ibge.cidade, 
#     (strftime('%Y', dash_tab_vig_sinasc.dt_nasc) || '-' || 
#         CASE 
