...
- **Pre-publish (not yet on Julia General; single maintainer, ~4 internal apps, no external users):** breaking changes are cheap — get the API/schema/naming *right* over backward compatibility; deprecation shims (e.g. the `bulk_update` legacy-`filters` error) are internal migration aids to remove before publish; release-gating decisions carry the [`pre-publish` label](https://github.com/PingoLee/PormG.jl/issues?q=is%3Aopen+label%3Apre-publish); the publish gate is that query coming back empty. **"Cheap" is a measurement, not an assumption — take it before trading correctness away for compatibility.** Clone/grep the actual consuming apps for the call pattern and report the number; a surface with zero real call sites costs nothing to redesign, and choosing a guard over the right API without that number is how an avoidable wart becomes permanent (#444: the CTE API turned out to have **0** `.with(` call sites across every consuming app, which is what made redesigning it the cheap option rather than the expensive one). *(Remove this bullet once published.)*
...
- **A single integration file is a valid target — the full suite is a release gate, not a per-issue tax.** Most `test/integration/test_*.jl` open with `if !isdefined(Main, :PormG) include("common_setup.jl") end`, so `julia --project=test/integration test/integration/<file>.jl` runs one file against the already-seeded database, skipping the ~170-statement DDL bootstrap and fixture reseed that `runtests.jl` re-runs every time. Which files are exempt, and which diffs still owe a full run, are in [`pormg-issue-workflow`](../skills/pormg-issue-workflow/SKILL.md) → *Verify*; the full-suite-on-both-engines gate is precondition 4 of [`pormg-cut-release`](../skills/pormg-cut-release/SKILL.md).
