...
Use `git diff --name-only` first to learn the surface area (always available on any shell). The VS Code tool `get_changed_files` also works inside VS Code chat, but it is a deferred tool — load it with `tool_search` before calling it; do not call it directly.

Then inspect patches in ordered slices. The `:(exclude)` pathspec syntax is a git feature that works on both Linux/bash and Windows/PowerShell as long as Git is on `PATH`, but quoting differs by shell:
...
git diff -- src
git diff -- test
# PowerShell does not expand single-quoted pathspecs the same way;
# use separate per-folder calls instead of exclusion patterns:
