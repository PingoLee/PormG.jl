...
- **Versioning (`0.y.z`) and the rollout log.** `y` bumps on any breaking/behavior change, `z` on everything else — Julia's Pkg treats the `y` slot as major pre-1.0, so a consumer's `compat = "0.y"` accepts `0.y.*` and rejects the next breaking release. A breaking change is **not done** until it (1) bumps `version` in `Project.toml` (the `y` slot) **and** (2) prepends a **version-stamped** entry to [`UPGRADING.md`](../../UPGRADING.md) — the concrete `before → after` and the per-app rollout table. `UPGRADING.md` is the single source of truth for consuming-app agents (its header carries the apply recipe); do not mirror it into a second changelog file. Entries are version-stamped from `0.2.0` onward; older entries are pre-`0.2` history.
