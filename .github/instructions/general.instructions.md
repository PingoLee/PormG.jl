...
| `src/migrations/column_spec.jl` | The canonical column IR the migration diff runs on (#507). One compiler, `column_spec(field, conn)`, applied to **both** sides — declared and introspected — so a lossy reader choice stops mattering: every struct that renders the same compiles the same, by construction, because it renders through `Dialect._get_column_type`. Engine equivalence (SQLite `BIGINT ≡ INTEGER`, `UUID`/`JSON` ≡ `TEXT`) is decided in `parse_canonical_type`, **once**. Holds the single attribute classification, `NON_DB_ATTRS` / `SCHEMA_ATTRS`, named after Django's `Field.non_db_attrs`; `test/unit/test_column_spec.jl` fails when a `PormGField` gains a slot it neither reads nor classifies |
