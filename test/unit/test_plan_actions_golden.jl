...
1. **The golden plan corpus.** 36 (declared, live) column pairs driven end to end through
   `get_migration_plan` on both engines — 72 rendered plans, pinned statement for statement. 32 of
   ...
   after with the reason (see `Deliberate differences` below). The remaining 4 pairs were added
   after review, to cover a hazard nothing here could see — see `Post-review coverage`.
...
@testset "Plan actions from the typed delta (#507 phase 2)" begin
  ...
  # ───────────────────────────────────────────────────────────────────────────
  # Golden plan: the rendered plan for 32 column pairs on both engines
  ...
  @testset "every column-delta slot has a renderer branch" begin
    ...
    for (slot, (new_spec, expected)) in cases
      ...
      sql = @test_logs min_level = Logging.Warn Dialect.alter_field(
        GPG507, "child_t", "col", Models.IntegerField(), Models.IntegerField(), delta)
    ...
    @test @test_logs(min_level = Logging.Warn,
                     Dialect.alter_field(GPG507, "child_t", "col", Models.IntegerField(),
                                         Models.IntegerField(), ref_delta)) == ""
    ...
    @test Dialect.alter_field(GPG507, "child_t", "col", Models.IntegerField(),
                              Models.IntegerField(), ColumnDelta(base, base, Symbol[])) == ""
  end
  ...
  @testset "a database-side expression default is emitted verbatim" begin
    ...
    sql = @test_logs min_level = Logging.Warn Dialect.alter_field(
      GPG507, "child_t", "col", Models.DateTimeField(null = true), Models.DateTimeField(null = true),
    ...
    lit_sql = Dialect.alter_field(GPG507, "child_t", "col", Models.DateTimeField(null = true),
                                  Models.DateTimeField(null = true), ColumnDelta(lit, base, [:default]))
