...
The right-hand side of an `F` comparison may be an integer or a float, a string, a `Base.UUID`, or one
of the temporal types `Dates.Time`, `Dates.Date`, `Dates.DateTime` and `TimeZones.ZonedDateTime`. Whatever
...
comparison itself, naming the type and the accepted ones. With the `F` on the left, a comparison never
silently evaluates to a `Bool`. To test for `NULL`, filter with `"col__@isnull" => true` instead.
