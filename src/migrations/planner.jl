...
function _drop_fk_constraint_in_alteration(conn::Union{PormGPostgres, PormGSQLite}, migration_plan::OrderedDict{Symbol, OrderedDict{String, String}}, model_name::Symbol, field_name::String, new_field::PormGField, old_field::PormGField)::Nothing
...
function _drop_fk_constraint_in_alteration(conn::Union{PormGPostgres, PormGSQLite}, migration_plan::OrderedDict{Symbol, OrderedDict{String, String}}, model_name::Symbol, field_name::Symbol, new_field::PormGField, old_field::PormGField)
...
function _drop_index(conn::Union{PormGPostgres, PormGSQLite}, migration_plan::OrderedDict{Symbol, OrderedDict{String, String}}, model_name::Symbol, field_name::String; index_name::Union{String, Nothing} = nothing)::Nothing
  if conn isa PormGSQLite
    # For SQLite, we might need a different way to get index name if not provided
    if index_name === nothing
       # TODO: implement get_constraints_index for SQLite if needed, 
       # but SQLite often uses table_name_field_name_idx convention or we can find it in sqlite_master
       return nothing 
    end
  else
...
function _alter_table_fields(conn::Union{PormGPostgres, PormGSQLite}, migration_plan::OrderedDict{Symbol, OrderedDict{String, String}}, model_name::Symbol, model::PormGModel, current_schema::Dict{Symbol, Dict{Symbol, Union{Bool, PormGModel}}}, settings::SQLConn)::Nothing
...
function makemigrations(connection::PormGSQLite, settings::SQLConn; path::String = "db/models.jl")
  if !settings.change_db
    @warn("The database is not set to change_db, so the migration plan will not be applied.")
    return
  end
