...
| `InvalidMigrationError` | A duplicate index name in a plan, an invalid answer to an interactive `makemigrations` prompt or end of input at one, or an unimplemented `migrate_to(version)` path. |
