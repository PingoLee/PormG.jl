...
# Returns a Dict with rich metadata (:dialect, :model, :bucketing, :operation etc.)
println(inspection[:sql_text])    # Print just the SQL
println(inspection[:operation])   # Heuristically detected (e.g., :select)
```
...
results = query.list(show_query=:execute)

# :sql -> Returns just the SQL string (Minimal overhead for benchmarking builder)
sql_only = query.list(show_query=:sql)
...
sql = bulk_insert(df_data, show_query=:sql)
...
| **`:sql` Mode** | Returns SQL **string** | N/A |
| **`:dict` Mode** | Returns full metadata | Standard return format |
| **`:none` Mode** | Builder run with zero-return | N/A |
| **Intent Detection** | Manual override | Heuristic detection |
| **Scope** | Works for SELECT, INSERT, UPDATE, DELETE | Works for all operations |
