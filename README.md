...
- `Decimals` is declared as `"0.4, 0.5"` rather than the newest major alone, and that is deliberate: **every** `LibPQ` release through 1.18.0 requires `Decimals 0.4`, so declaring `"0.5"` alone leaves the only PostgreSQL driver with no installable version and any app depending on both fails to resolve ([#558](https://github.com/PingoLee/PormG.jl/issues/558)). PormG builds a `Decimals.Decimal` only through the three-argument `Decimal(sign, coefficient, exponent)` constructor, the one spelling both majors share — `parse(Decimal, "1.23456789e6")` throws on `0.4.1`, and `0.5` prints a positive exponent as `1E+1` — so an environment carrying `LibPQ` resolves `0.4.x`, a driver-free or SQLite-only one resolves `0.5.x`, and PormG works the same either way ([#648](https://github.com/PingoLee/PormG.jl/issues/648))
- `OrderedCollections` is declared as `"1, 2"` rather than the newest major alone, and that is deliberate too: two of the internal apps depend on a package that caps `OrderedCollections` at 1 — `XLSX` accepts 2 only from `0.12`, `Genie` only from `6` — so declaring `"2"` alone makes those apps unresolvable the moment they bump PormG, a break no PormG environment can observe ([#560](https://github.com/PingoLee/PormG.jl/issues/560)). PormG constructs `OrderedDict` / `OrderedSet` explicitly **and with the element type spelled out** everywhere, and uses no API that differs between the majors ([#549](https://github.com/PingoLee/PormG.jl/issues/549)), so an app whose graph caps it resolves `1.x`, an unconstrained one resolves `2.x`, and PormG works the same either way. Spelling the element type is not decoration: `OrderedDict(k => v for …)` only infers a concrete type from OC `1.3` — below that it yields `OrderedDict{Any, Any}` — so the version that bites is a *minor*, not the `1`/`2` major boundary everyone watches ([#574](https://github.com/PingoLee/PormG.jl/issues/574))
