...
| `bulk_update` on a handler with `limit`/`offset`/`order_by`/`distinct`/aggregates, a CTE, or a `cjoin` | `UnsafeMutationError` | An `UPDATE` cannot express them; the handler's plain filters are kept — see [Bulk Operations](write/bulk.md) |
