...
# The `:expression_default` findings — the columns whose DEFAULT the readers drop because it is a
# SQL expression rather than a literal (#475).
#
# Decided with the READERS' OWN pure helpers rather than a mirror of them (#522): `_clean_default` is
# the classification `_default_or_drop` applies before it decides to drop, and `_key_arm` the arm
# selection both readers use, so `check` cannot disagree with `makemigrations` about which columns
# are affected. This replaced two per-engine copies of the readers' arm order (`_pg_key_arm_ignores_
# default`, `_sqlite_key_arm_ignores_default`) whose own comment said they could drift, and the
# agreement assertions in `test/unit/test_migrations_check.jl` and
# `test/integration/test_importers_introspection.jl` (section 6b) are what would have caught it.
#
# The bare-`IDField` key arm is skipped. That arm is precisely an INTEGER column on the fall-through
# key arm (`_integer_key_arm`; a TEXT key landing there reads its default like any column), and the
# reason for the skip is narrower than it looks — it is NOT that
# `IDField` expresses a `serial` column's `nextval(…)` (it does not: a legacy `serial` key reads back
# `generated = false` and `makemigrations` proposes `ADD GENERATED BY DEFAULT AS IDENTITY` for it,
# UPGRADING.md #438). It is that `sIDField.default` is `Union{Int64, Nothing}`, so that arm reads no
# default at all and a user CANNOT have declared one there: `check`'s one finding class says "your
# model must declare no `default=` on this column", which is already true by construction for every
# `IDField`, and so would be noise rather than a finding. Erring toward REPORTING is deliberate
# everywhere else: `check` being silent about a column the reader really did drop is the dangerous
# direction, and being noisy is merely annoying.
function _expression_default_finding(table_name, col_name, raw_default, ctype, is_pk::Bool,
  ...
  _integer_key_arm(_key_arm(is_pk, ctype, has_reference), ctype) && return nothing
