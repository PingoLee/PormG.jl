...
- [/] **SQLite `INSERT ... RETURNING *` hang + worker/pool stability** (`src/querybuilder/execution.jl`, `src/ConnectionPool.jl`)
  ...
  - [ ] **SQLite connection-pool timeout in transactions** — "Timeout after 30s waiting for available SQLite connection" in `test_transactions.jl`; the pool grows past size 3 then exhausts after `run_in_transaction` exits. Runtime worker/pool accounting issue, not the query builder. **Reproduced 2026-06-23** (`$env:PORMG_DB="db_sl"; julia -t 1 ... runtests.jl`): pool expanded 3→4, then timed out right after the `with_transaction` block — the run stops there. **Resize vs. wait**: raising the wait/timeout does *not* help; `pool_size` is the lever ([Configuration.jl:220](src/Configuration.jl#L220), default 3). **But `*connection.yml` is git-ignored** (`.gitignore:3`; `db_sl/connection.yml` is untracked), so setting `pool_size:` there is a **local diagnostic only — it does not ship**: a fresh checkout regenerates the yml, falls back to default 3, and the timeout returns. Use a one-off `pool_size: 12` *only* to discriminate the cause — if a bigger pool clears it → transient contention (a transaction pins one connection while a nested op acquires another); if it only *delays* the timeout → a connection is leaked (a tx/`fetch_async` connection never released). The **durable** fix must be committed: either (a) root-cause the leak/contention so the default 3 works (preferred; overlaps the PG item above), or (b) set the test pool size in committed code — e.g. an override in `test/integration/common_setup.jl` after `Configuration.load`, or a committed default — never in the ignored yml.
