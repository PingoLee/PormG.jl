...
`generate_models_from_db` reads each live column into the same canonical description `makemigrations` diffs on, and only then picks the field to write. Where the column says exactly what it is, the choice is mechanical: `varchar(40)` is `CharField(max_length = 40)`, `numeric(8, 3)` is `DecimalField(max_digits = 8, decimal_places = 3)`, a `bytea`/`BLOB` whose CHECK is exactly `octet_length(col) <= n` (`length(col) <= n` on SQLite, the one PormG writes) is `BinaryField(max_length = n)` (a compound bound such as `CHECK (octet_length(photo) <= 1048576 AND octet_length(photo) > 0)` is your own constraint and leaves a `BinaryField()`), a `timestamp without time zone` is `DateTimeField(type = "TIMESTAMP")`, an integer whose CHECK is exactly `col >= 0` (the one PormG writes) is a `PositiveIntegerField` (a range such as `CHECK (grid >= 0 AND grid <= 30)` is your own constraint and leaves an `IntegerField`), an integer key is an `IDField`, a foreign key is a `ForeignKey` — or a `OneToOneField` when the column is unique or is the primary key.
...
- By default a column equals its field name. To map a field to a **differently-named** column, set
  ...
  **referenced** parent column follows `pk_field`, resolved through the parent field's own
  ...
  !!! note "A CTE exposes aliases, not columns"
      ...
      (`"product_sku" AS "sku"`). So `"ev__sku"` names the **field** (or its custom alias),
      ...
      That column can be written either way — `"ev__sku"` or `CTE("ev", "sku")` — and both name the
      projection alias. The object is the disambiguator: a CTE's columns are a namespace of their
      own, but both namespaces are reached by a `__` path, so a CTE name that *also* names a model
      field makes the shared path raise `AmbiguousFieldError` and `CTE(name, path)` is how you
      select the CTE side ([#444](https://github.com/PingoLee/PormG.jl/issues/444),
      [#492](https://github.com/PingoLee/PormG.jl/issues/492)). See
...
An **expression** default — `DEFAULT now()`, `DEFAULT CURRENT_DATE`, `DEFAULT gen_random_uuid()`,
`DEFAULT (random() * 10)::integer` — is recognised as an expression while the schema is being read,
on **every** column type and both engines. It is stored in a slot of its own, `db_default`, and
rendered back verbatim.

```julia
# live: created_at TIMESTAMPTZ NOT NULL DEFAULT now()
created_at = Models.DateTimeField(null=false, db_default=(postgres="now()",))
```

`default=` and `db_default=` are **mutually exclusive**, and declaring both raises
`FieldValidationError`. They mean genuinely different things: `default=` is a value **PormG** writes
when it inserts the row (and renders into the column definition), while `db_default=` is an
...
| You write | Meaning |
|---|---|
| `db_default="CURRENT_TIMESTAMP"` | portable — rendered on both engines |
| `db_default=(postgres="now()",)` | pinned — rendered on PostgreSQL, **raises** on SQLite |
| `db_default=(postgres="now()", sqlite="CURRENT_TIMESTAMP")` | a spelling for each engine |
| `db_default=(postgres="gen_random_uuid()", sqlite=nothing)` | PostgreSQL only; no default on SQLite |

A bare string is accepted **only** for the two expressions that round-trip identically through both
catalogs — `CURRENT_TIMESTAMP` and `CURRENT_DATE`. Anything else must name its engine:

```julia
Models.DateTimeField(db_default="now()")     # FieldValidationError: name the engine
```

Migrating a pinned column on the other engine raises `BackendCapabilityError` rather than emitting
DDL that database would reject. That is deliberate — PormG will not guess a translation — and the
message names both remedies: add the other engine's spelling, or `sqlite=nothing` to declare that
the column simply has no database default there.

!!! warning "The expression must be spelled the way the catalog reports it"
    PormG compares the declared expression to the live one **as text**, and PostgreSQL does not
    store your text — it stores a parse tree and re-prints it. `DEFAULT (random() * 10)::integer`
    is held in the catalog as `((random() * (10)::double precision))::integer`, and `check()` reports
    it as `(random() * (10)::double precision)`. If what you declare and what the catalog reports
    differ, `makemigrations` proposes `SET DEFAULT` on every run and never converges.

    The reliable procedure is to let PormG tell you: create the column, then run
    `PormG.Migrations.check("db")` (below) or `inspectdb`, and paste the text it reports. Anything
    generated by `inspectdb` is already in that form.

    This is the price of taking a raw string. Django's `db_default` takes an expression *object*
    compiled per backend and so compares objects instead; PormG is deliberately less magic here,
    and the two portable spellings above exist precisely so the common cases are immune.

!!! note "SQLite adds the parentheses for you"
    SQLite's grammar requires `DEFAULT (expr)` for anything outside its small `literal-value` set,
    and `PRAGMA table_info` then reports the text back with that layer removed. Write the bare
    expression — `db_default=(sqlite="abs(random()) % 10",)` — and PormG adds the parentheses when
    it renders. Writing them yourself is harmless; they are normalised away.

Two consequences worth knowing before you go looking for them:

**PormG will not propose dropping a default your model does not mention.** A live expression default
against a model that declares none is deliberately *not* a difference — no migration is proposed,
and no `DROP DEFAULT` is generated against your live `now()`. Adopting an existing database
therefore stays a no-op until you choose to describe the column. If you want the database default
gone, remove it by hand, or declare it with `db_default=` first and then change it.

The same rule means removing a `db_default=` you previously declared proposes nothing either.
A state-based engine has no history to distinguish "never declared" from "deliberately removed",
...
Reach for `db_default=` when you want the **database** to own the value — because rows arrive from
outside PormG too, or because the expression is something PormG has no field for
(`gen_random_uuid()`, a sequence, a computed literal).

### `db_default` is rendered verbatim, and that is a deliberate exception

PormG never interpolates user input into SQL; every value in a query is a bound parameter. A
`db_default` is the documented exception, alongside `db_table` and `db_column`: it is
**author-supplied schema text**, written in a models file by the person who controls the schema, and
it is rendered into DDL exactly as written. PormG does not parse it, validate it as SQL, or escape
it — a syntactically invalid expression fails at the database.

Introspection may also populate it, from `pg_get_expr` or `PRAGMA table_info` output. That text is
the catalog's own re-printing of a **parsed** expression, so it cannot smuggle a statement
...
note_b TEXT DEFAULT now()      -- an EXPRESSION. Kept, as a db_default.
```

`note_a` imports as `Models.TextField(default="now()")` and round-trips exactly. `note_b` imports as
`Models.TextField(db_default=(postgres="now()",))` — the same five characters, described as an
expression rather than as a value, and it round-trips too.
...
`IDField` is reconstructed from the key itself, and *any* default on it is discarded without a
warning. `IDField` has neither a `default` that could hold an expression (it is an integer or
nothing) nor a `db_default` slot at all: PostgreSQL rejects a column that is both
`GENERATED … AS IDENTITY` and carries a `DEFAULT`, so giving it one would make invalid DDL
declarable. `check()` stays quiet about these for the same reason — reporting one would be advice
you could not follow.

That covers more than the ordinary integer/`serial` case: **every** key on the fall-through arm
imports as an `IDField`, including a `BLOB` or a `REAL` one. The single exception is a lengthless
`TEXT` key on SQLite, which imports as `UUIDField(primary_key=true)` and does carry its expression.
A key that imports as a `UUIDField`, a `CharField` or a relation follows the normal rule above.

!!! warning "Use `db_default=`, never `default=`, for an expression"
    Copying the reported expression into `default=` is the one response that causes damage:

    ```julia
    # ✗ never — a VALUE, rendered as DEFAULT 'now()'
    created_at = Models.DateTimeField(default="now()")

    # ✓ an EXPRESSION, rendered as DEFAULT now()
    created_at = Models.DateTimeField(db_default=(postgres="now()",))
    ```

    `default=` is a literal. Declaring `default="now()"` makes `makemigrations` propose
    `ALTER TABLE … SET DEFAULT 'now()'`, a **quoted literal**, which replaces the database's real
    expression default with a constant string — every new row then stores those five characters
    instead of a timestamp. On PostgreSQL that statement is not classified as destructive, so
    nothing prompts before it runs.

    The two slots are mutually exclusive, so this is a mistake you can only make by choosing the
    wrong one, never by setting both.

### Listing the columns: `check()`

To ask which columns have an expression default — at any time, against the live database, without a
...
`check()` is read-only and works on both engines. **The text it prints is the text to paste** — it
is the catalog's own spelling, which is what the declared side has to match for the column to
converge. That makes it the pre-flight step before writing models against an existing database, and
the fix for a column that keeps re-planning.

The two things it is telling you about such a column: declare it as `db_default=` and not as
`default=`, and — unless the expression is `CURRENT_TIMESTAMP` or `CURRENT_DATE` — it is pinned to
this engine, so a portable app needs a spelling for the other one.

`isempty(result)` is true when there is nothing to report; `result.findings` carries a
`SchemaCheckFinding` per column for programmatic use. It sits alongside `status()` and `dry_run()`
