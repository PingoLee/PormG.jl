...
- **Merge gate — the PR is the review point, and it is the only gate on the happy path.** Plan approval (including `ExitPlanMode`) authorizes the **whole run**: implement, verify at the tier's rungs, review, `git commit`, `git push`, open the PR. No per-step approval, no stopping to show a diff and wait. **Never merge**, and never close an issue by hand — put `Closes #N` in the PR body (only when the PR actually completes the issue) and let the maintainer's merge do it.
  ...
  - **Integration runs: the suite lock arbitrates the database; the user arbitrates what the lock cannot.** A rung-4 **slice** runs **without asking** — on `db_2` or on a worktree's `db_sl` — because plan approval covers it like any other rung and the suite lock in `common_setup.jl` queues a second `db_2` run instead of interleaving it (*Verification*, below). A lock timeout is a result to report, never a reason to opt out of the lock. **Ask first, every time, whatever the plan says**, for the four cases the lock does not arbitrate:
    - the **full** `test/integration/runtests.jl`, either engine — minutes of CPU and memory on the maintainer's machine, and a truncate-and-reseed that leaves the fixture half-seeded for every later slice if it is cut short;
    - anything that **bypasses the lock** — `PORMG_TEST_LOCK=0`, a `psql` session, a scratch script that does not include `common_setup.jl`;
    - a repro **hostile** to other sessions — terminating another backend, restarting the server, dropping a schema, wiping the fixture: the lock queues *runs*, it does not protect *connections*;
    - `db_sl` in the **main checkout** — only a worktree's `f1.sqlite` is private, and SQLite is exempt from the lock.

    A subagent gets **no** database access: a review brief says so explicitly, and a finding that needs a live check is run by the parent session, under these same rules. This rests on one premise — **`db_2`'s only clients are PormG test sessions**. The lock is advisory, so if `db_2` is ever pointed at a server something else writes to, every run is asked again. It replaced an ask-before-every-run rule written while `db_2` was a shared remote server and concurrent runs interleaved; the lock (`5b46c60a`) and a local `db_2` retired both halves of that premise.
